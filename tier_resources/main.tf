locals {
    adxCluster = lower("adxjupitershared${var.vmEnv}${var.region}01")
}
...
module "backup_policy" {
    ...
    environment               =  var.environment 
    # sql_policy_count          =  "1"
    # sql_backup_policy_name    =  module.app_sql_backup.concatenated_name
}
module "app_sql_backup" {
    source                    = "./terraform_naming_module"
    resource_type_input       = "backup_policy"
    business_unit_input       =  var.business_unit 
    workload                  =  var.workload 
